...
const Home: NextPage = () => {
  ...
  const mintSword = async () => {
    ...
    try {
      await writeContractAsync({
        abi: deployedContracts[8453]?.Sword?.abi,
        address: deployedContracts[8453]?.Sword?.address,
        functionName: "mint",
        args: [user?.wallet?.address],
      });
    } catch (error) {
      console.error("Error minting Sword:", error);
    }
  };
  ...
  const mintStaff = async () => {
    ...
    try {
      await writeContractAsync({
        abi: deployedContracts[8453]?.Staff?.abi,
        address: deployedContracts[8453]?.Staff?.address,
        functionName: "mint",
        args: [user?.wallet?.address],
      });
    } catch (error) {
      console.error("Error minting Staff:", error);
    }
  };
  ...
  const mintBow = async () => {
    ...
    try {
      await writeContractAsync({
        abi: deployedContracts[8453]?.Bow?.abi,
        address: deployedContracts[8453]?.Bow?.address,
        functionName: "mint",
        args: [user?.wallet?.address],
      });
    } catch (error) {
      console.error("Error minting Bow:", error);
    }
  };
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
              {nftMetadata.Sword && (
                <div className="bg-base-200 p-4 rounded-lg">
                  ...
                  <div className="mt-4 text-center">
                    <button onClick={mintSword} disabled={!user?.wallet?.address} className="btn btn-primary">
                      Mint
                    </button>
                  </div>
                </div>
              ...
              {nftMetadata.Staff && (
                <div className="bg-base-200 p-4 rounded-lg">
                  ...
                  <div className="mt-4 text-center">
                    <button onClick={mintStaff} disabled={!user?.wallet?.address} className="btn btn-secondary">
                      Mint
                    </button>
                  </div>
                </div>
              ...
              {nftMetadata.Bow && (
                <div className="bg-base-200 p-4 rounded-lg">
                  ...
                  <div className="mt-4 text-center">
                    <button onClick={mintBow} disabled={!user?.wallet?.address} className="btn btn-accent">
                      Mint
                    </button>
                  </div>
                </div>
